...
# modification to save box proposals
detectron2/detectron2/modeling/meta_arch/rcnn.py
add self.proposals = None in the beginning, then in inference, after proposals are generated, add self.proposals = proposals

## Questions:
for bootstrap I had to mkdir bootstrap_dfs.
in src/bootstrap, there is a TODO that says bootstrap should sample with replacement
...
## detectron2
(*TODO*: is it possible to have a script that does this? It can ask the users, we are going to checkout detectron and set it up, do you wish to proceed something like this)
...
Note: to run on cpu only, add MODEL.DEVICE cpu
...
or run `make patch` to run `patch.sh`, `patch.sh` will add the line automatically. (Mac users run `brew install coreutils` first to install realpath)
